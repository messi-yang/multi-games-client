...
export function useHotKeys(
  keys: string[],
  options: { onPressedKeysChange?: (keys: string[]) => void; onKeyUp?: (key: string) => void; onKeyDown?: (key: string) => void }
) {
  ...
  const keyDownHandler: (this: Window, ev: KeyboardEvent) => void = useCallback(
    ({ code }) => {
      if (keys.indexOf(code) > -1 && keysPressed.indexOf(code) === -1) {
        ...
        options.onKeyDown?.(code);
      }
    },
    [options.onPressedKeysChange, options.onKeyDown, keysPressed]
  );
  ...
  const keyUpHandler: (this: Window, ev: KeyboardEvent) => void = useCallback(
    ({ code }) => {
      if (keys.indexOf(code) > -1 && keysPressed.indexOf(code) > -1) {
        ...
        setKeysPressed(newKeysPressed);
        options.onPressedKeysChange?.(newKeysPressed);
        options.onKeyUp?.(code);
      }
    },
    [options.onPressedKeysChange, options.onKeyUp, keysPressed]
  );
